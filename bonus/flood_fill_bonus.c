...
// 		return ;
// 	if (game->map_array[(player_y / PIXEL)][(player_x / PIXEL)] == EXIT
...
void	flood_fill_enemy(t_game *game)
{
	...
	player_y = game->o.hero_i->instances->y;
	player_x = game->o.hero_i->instances->x;
	map_y = (game->map_pos.y - 1) * PIXEL;
	map_x = (game->map_pos.x - 2) * PIXEL;
...
void	flood_fill(t_game *game, int x, int y, int *collectible)
		...
		void flood_fill_init(t_game *game)
		{
			...
			flood_fill(game, game->character.x, game->character.y,
					&collectible);
			if (game->nb_exit != 0 || collectible != 0)
